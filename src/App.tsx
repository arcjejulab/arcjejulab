...
>
  ...
  <br className="hidden md:block" />
  방법을 찾기 전에, 이 매장이 어떤 매장인지 먼저 물어야 합니다.
  <br className="hidden md:block" />
  도구를 사용하기 전에, 브랜드가 나아갈 방향을 먼저 정해야 합니다.
</p>
            </motion.div>
          </div>
        </section>
...
const BrandDirectionPage = () => {
 ...
 const concerns = [
  '매장의 장점을 한마디로 설명하기 어려울 때',
  '메뉴와 공간, 콘텐츠가 서로 다른 분위기를 보일 때',
  '경쟁 매장과의 차이가 분명하지 않을 때',
  '광고와 콘텐츠를 반복해도 고객에게 남는 인상이 흐릿할 때',
  '새로운 메뉴와 이벤트를 계속 진행해도 효과가 나타나지 않을 때',
  '직원마다 고객을 응대하는 방식이 다를 때',
];
